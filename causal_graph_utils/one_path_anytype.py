...
def one_path_anytype(matrix, start, end, path_=[]):
    '''
    Recursive function to search for at least one path of any type from 'start' node to 'end' node
    ...
    neighbors = np.where(np.logical_and(matrix[start, :] != 0, matrix[:, start] != 0))[0]
    ...
    if end in neighbors:
        path = one_path_anytype(matrix, end, end, path_)
        return path
    else:
        for node in neighbors:
            if node not in path_:
                path = one_path_anytype(matrix, node, end, path_)
                if path:
                    return path
        return None
